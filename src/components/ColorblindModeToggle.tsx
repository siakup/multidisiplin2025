...
export default function ColorblindModeToggle({ fixed = false }: ColorblindModeToggleProps) {
  ...
  // Warna icon: normal = hitam, buta warna = abu-abu agak ke hijauan
  const iconClass = isEnabled ? 'text-gray-400' : 'text-black';

  return (
    <div className={containerClass}>
      <button
        onClick={handleToggle}
        className="relative inline-flex items-center justify-center w-10 h-10 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-brand-text-primary focus:border-transparent transition-colors shadow-sm"
      ...
      >
        {/* Icon Kacamata */}
        <svg
          className={`w-6 h-6 transition-colors ${iconClass}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        ...
        {isEnabled && (
          <span className="absolute -top-1 -right-1 w-3 h-3 bg-brand-text-primary rounded-full border-2 border-white"></span>
        )}
      </button>
    </div>
  );
