...
export default function ElectricityBillsEditPage() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (showNamaPanelDropdown && !target.closest('.relative.flex-1')) {
        setShowNamaPanelDropdown(false);
      }
    };

    if (showNamaPanelDropdown) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validasi nama panel harus dipilih
      if (!formData.namaPanel || !formData.panelId) {
        throw new Error('Silahkan pilih nama panel');
      }
